...
class Zoom
{
    ...
    public function __construct(array $config = [])
    {
      ...
      $this->CREDENTIAL_DATA = json_decode(json_decode(Storage::get($this->CREDENTIAL_PATH), true), true);
    }
    ...
    public function token($code)
    {
      ...
      Storage::put($this->CREDENTIAL_PATH, json_encode($token));
      // file_put_contents($this->CREDENTIAL_PATH, $token);
      if (!Storage::exists($this->CREDENTIAL_PATH)) {
        return ['status' => false, 'message' => 'Error while saving file'];
      }
      $savedToken = json_decode(json_decode(Storage::get($this->CREDENTIAL_PATH), true), true); //getting json from saved json file
    ...
    public function refreshToken()
    {
      try {
        ...
        $response_token =json_decode($response->getBody()->getContents(), true);
        $token = json_encode($response_token);
        Storage::put($this->CREDENTIAL_PATH, $token);
        if (!Storage::exists($this->CREDENTIAL_PATH)) {
          throw new Exception("Token file not exist");
        }
        $savedToken = json_decode(Storage::get($this->CREDENTIAL_PATH), true); //getting json from saved json file
        if (!empty(array_diff($savedToken,$response_token))) { // checking reponse token and saved tokends are same
